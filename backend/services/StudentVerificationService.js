...
export class StudentVerificationService {
  ...
  async requestVerification(studentUserId, schoolId) {
    ...
    return withTransaction(async (connection) => {
      return this.verificationRepo.createRequest({ studentId: student.id, schoolId }, connection);
    });
  }

  async decide(verificationId, decision, schoolUserId) {
    const allowed = ['APPROVED', 'REJECTED'];
    if (!allowed.includes(decision)) throw new Error('Transición inválida');
    ...
    return withTransaction(async (connection) => {
      ...
      if (verification.status !== 'PENDING') throw new Error('Transición no permitida');
